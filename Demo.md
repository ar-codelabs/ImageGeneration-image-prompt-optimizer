...
subgraph SCULPTOR["PromptSculptor 내부"]
...
subgraph LOOP["반복 루프 (max iterations)"]

E[Nova Canvas 이미지 생성\nCOLOR_GUIDED_GENERATION] --> F

F[DVQ 평가\nNova Pro Vision이 각 질문 Yes/No 판정] --> G

G{Best 선택}

G -->|DVQ +5%p 이상| H1[✅ DVQ 우선 업데이트]

G -->|DVQ 비슷| H2[Pairwise 비교\n2n번 순서 교체 비교]

G -->|DVQ -5%p 이하| H3[↩️ 기존 Best 유지]
...
J([최종 결과\nBest 이미지 + DVQ 점수 + 프롬프트])

  

style INIT fill:#e8f4f8,stroke:#2196F3

style SCULPTOR fill:#fff3e0,stroke:#FF9800

style LOOP fill:#f3e5f5,stroke:#9C27B0
...
subgraph LLM["Amazon Bedrock LLM (us-east-1)"]
...
subgraph IMG["Amazon Bedrock 이미지 (eu-west-1)"]
...
NP -->|"① 스타일 분석 프롬프트 생성"| MERGE

NL -->|"② 핵심 요소 프롬프트 생성"| MERGE

MERGE["③ Nova Pro\n프롬프트 결합"] --> NC

NC -->|"④ 이미지 생성"| EVAL

EVAL["⑤ Nova Pro Vision\nDVQ 평가 + Pairwise 비교"] -->|"⑥ 개선 필요"| NP

EVAL -->|"⑦ Best 확정"| OUT([최종 이미지])
